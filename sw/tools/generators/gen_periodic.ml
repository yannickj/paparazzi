...
let get_sys_telemetry_settings = fun telemetry ->
  ...
  | Some telemetry ->
    ...
    let tl_settings = List.fold_left (fun lp p ->
      if List.length p.Telemetry.Process.modes > 1 then begin
        (* only if more than one mode *)
        let p_name = p.Telemetry.Process.name in
        let modes = List.map (fun m -> m.Telemetry.Mode.name) p.Telemetry.Process.modes in
        ...
        match nb_modes with
          ...
          | _ -> (* add settings with all modes *)
              let (key, _) = List.fold_left (fun (lk, i) m ->
                match m.Telemetry.Mode.key_press with
                | None -> (lk, i+1)
                | Some k -> ((Xml.Element ("key_press", [("key", k); ("value", string_of_int i)], [])) :: lk, i+1)
              ) ([], 0) p.Telemetry.Process.modes in
    ...
    ) [] telemetry.Telemetry.processes in
