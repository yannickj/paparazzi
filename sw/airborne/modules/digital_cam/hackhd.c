...
static inline void hackhd_log_shot_position(void)
{
  ...
  if (pprzLogFile.fs != NULL && hackhd.photo_nr > 0) {
    ...
    sdLogWriteLog(&pprzLogFile, "%d %d %d %d %d %d %d %u %u\n",
        hackhd.photo_nr,
        ...
        (int32_t)(pos.east*100.0f),
        (int32_t)(pos.north*100.0f),
        (int32_t)(pos.alt*100.0f),
        (uint32_t)pos.zone,
        time);
  }
}
...
void hackhd_periodic( void ) {
  if (hackhd.timer) {
    ...
    HACKHD_RELEASE(HACKHD_GPIO);
    //if (hackhd.photo_nr == 0 && hackhd.status == HACKHD_POWER_ON) {
