...
/**
 * @file modules/ins/ins_mekf_wrapper.c
 *
 * Paparazzi specific wrapper to run MEKF INS filter.
 */

#include "modules/ins/ins_mekf_wrapper.h"
...
//#define MEKF_USE_UTM TRUE
#if MEKF_USE_UTM
#include "firmwares/fixedwing/nav.h"
#endif

#ifndef INS_MEKF_FILTER_ID
#define INS_MEKF_FILTER_ID 3
#endif

struct InsMekf ins_mekf;

/** last accel measurement */
static struct FloatVect3 ins_mekf_accel;
...
static void send_euler(struct transport_tx *trans, struct link_device *dev)
{
  ...
  struct FloatQuat quat = ins_mekf_get_quat();
  float_eulers_of_quat(&ltp_to_imu_euler, &quat);
  uint8_t id = INS_MEKF_FILTER_ID;
...
static void send_filter_status(struct transport_tx *trans, struct link_device *dev)
{
  uint8_t mde = 3; // OK
  uint16_t val = 0;
  if (!ins_mekf.is_aligned) {
    mde = 2; // ALIGN
  } else if (!ins_mekf.baro_initialized || !ins_mekf.gps_initialized) {
    mde = 1; // INIT
  }
...
static void send_inv_filter(struct transport_tx *trans, struct link_device *dev)
{
  ...
  //struct FloatRates rates = ins_mekf_get_body_rates();
  struct NedCoor_f pos = ins_mekf_get_pos_ned();
  struct NedCoor_f speed = ins_mekf_get_speed_ned();
  //struct NedCoor_f accel = ins_mekf_get_accel_ned();
  struct FloatVect3 ab = ins_mekf_get_accel_bias();
  struct FloatRates rb = ins_mekf_get_rates_bias();
  float airspeed = 0.f;
...
/** baro */
#ifndef INS_MEKF_BARO_ID
#if USE_BARO_BOARD
#define INS_MEKF_BARO_ID BARO_BOARD_SENDER_ID
#else
#define INS_MEKF_BARO_ID ABI_BROADCAST
#endif
#endif
PRINT_CONFIG_VAR(INS_MEKF_BARO_ID)

/** IMU (gyro, accel) */
#ifndef INS_MEKF_IMU_ID
#define INS_MEKF_IMU_ID ABI_BROADCAST
#endif
PRINT_CONFIG_VAR(INS_MEKF_IMU_ID)

/** magnetometer */
#ifndef INS_MEKF_MAG_ID
#define INS_MEKF_MAG_ID ABI_BROADCAST
#endif
PRINT_CONFIG_VAR(INS_MEKF_MAG_ID)

/** ABI binding for gps data.
 * Used for GPS ABI messages.
 */
#ifndef INS_MEKF_GPS_ID
#define INS_MEKF_GPS_ID GPS_MULTI_ID
#endif
PRINT_CONFIG_VAR(INS_MEKF_GPS_ID)
...
static void baro_cb(uint8_t __attribute__((unused)) sender_id, float pressure)
{
  ...
  if (!ins_mekf.baro_initialized) {
    ...
    if (fabs(alpha) < 0.1f) {
      ins_qfe = baro_moy;
      ins_mekf.baro_initialized = true;
    }
    if (i == 250) {
      ins_qfe = pressure;
      ins_mekf.baro_initialized = true;
    ...
    if (ins_mekf.is_aligned && ins_mekf.gps_initialized) {
      float baro_alt = -pprz_isa_height_of_pressure(pressure, ins_qfe); // Z down
      ins_mekf_update_baro(baro_alt);
...
/**
 * Call ins_mekf_propagate on new gyro measurements.
 * Since acceleration measurement is also needed for propagation,
 * use the last stored accel from #ins_mekf_accel.
 */
                    ...
                    uint32_t stamp, struct Int32Rates *gyro)
{
  if (ins_mekf.is_aligned) {
    ...
    struct FloatRMat *body_to_imu_rmat = orientationGetRMat_f(&ins_mekf.body_to_imu);
    ...
    PRINT_CONFIG_MSG("Calculating dt for INS MEKF propagation.")

    if (last_imu_stamp > 0) {
      float dt = (float)(stamp - last_imu_stamp) * 1e-6;
      if (ins_mekf.gps_initialized) {
        ins_mekf_propagate(&gyro_body, &ins_mekf_accel, dt);
      } else {
        ins_mekf_propagate_ahrs(&gyro_body, &ins_mekf_accel, dt);
      }
    }
#else
    PRINT_CONFIG_MSG("Using fixed INS_PROPAGATE_FREQUENCY for INS MEKF propagation.")
    ...
    if (LogFileIsOpen()) {
      ...
      struct FloatVect3 ab = ins_mekf_get_accel_bias();
      struct FloatRates rb = ins_mekf_get_rates_bias();
      float bb = ins_mekf_get_baro_bias();
      PrintLog(pprzLogFile,
                     ...
                     struct Int32Vect3 *accel)
{
  ...
  struct FloatRMat *body_to_imu_rmat = orientationGetRMat_f(&ins_mekf.body_to_imu);
  // new values in body frame
  float_rmat_transp_vmult(&ins_mekf_accel, body_to_imu_rmat, &accel_f);
}
                   ...
                   struct Int32Vect3 *mag)
{
  if (ins_mekf.is_aligned) {
    ...
    ins_mekf_update_mag(&mag_body, !ins_mekf.gps_initialized);
                       ...
                       struct Int32Vect3 *lp_mag)
{
  if (!ins_mekf.is_aligned) {
    ...
    ins_mekf_align(&gyro_body, &quat);
    // udate state interface
    set_state_from_ins();

    // ins and ahrs are now running
    ins_mekf.is_aligned = true;
  }
}
...
{
  orientationSetQuat_f(&ins_mekf.body_to_imu, q_b2i);
  if (!ins_mekf.is_aligned) {
    // set ltp_to_imu so that body is zero
    ins_mekf_set_quat(q_b2i);
  }
}

static void geo_mag_cb(uint8_t sender_id __attribute__((unused)), struct FloatVect3 *h)
{
  ins_mekf_set_mag_h(h);
}
...
{
	if (ins_mekf.is_aligned && gps_s->fix >= GPS_FIX_3D) {

#if MEKF_USE_UTM // fixedwing firmware
 ...
 * Init function
 */
void ins_mekf_wrapper_init(void)
{
  // init position
#if MEKF_USE_UTM
  ...
  ins_mekf_set_mag_h(&mag_h);

  // Bind to ABI messages
  AbiBindMsgBARO_ABS(INS_MEKF_BARO_ID, &baro_ev, baro_cb);
  AbiBindMsgIMU_MAG_INT32(INS_MEKF_MAG_ID, &mag_ev, mag_cb);
  AbiBindMsgIMU_GYRO_INT32(INS_MEKF_IMU_ID, &gyro_ev, gyro_cb);
  AbiBindMsgIMU_ACCEL_INT32(INS_MEKF_IMU_ID, &accel_ev, accel_cb);
  AbiBindMsgIMU_LOWPASSED(INS_MEKF_IMU_ID, &aligner_ev, aligner_cb);
  AbiBindMsgBODY_TO_IMU_QUAT(INS_MEKF_IMU_ID, &body_to_imu_ev, body_to_imu_cb);
  ...
  uint32_t counter = 0;
  char filename[512];
  snprintf(filename, 512, "%s/mekf_%05d.csv", STRINGIFY(MEKF_LOG_PATH), counter);
  // check availale name
